...
def guide_get_feature(stub):
    #Generate UUID and Wrap in request_info by bracking with []
    uuid0 = str(uuid.uuid1())
    request_info = "some info you had before"
    request_info +="["+uuid0+"]"
    #Generate UUID Finished

    guide_get_one_feature(
        stub, route_guide_pb2.Point(latitude=409146138, longitude=-746188906, request_info=request_info))
    ...
    rlog0 = RedisLog(sname = 'route_guide_client', 
                         fname = 'guide_get_feature',
                         status = 0,
